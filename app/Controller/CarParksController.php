...
class CarParksController extends AppController {
    ...
    public function corkCarparks() {
        console.log("Scanning cork/carparks");
    ...
    public function corkBikes() {
         console.log("Scanning cork/bikes");
    ...
    public function totalParking($previousOrCurrent) {
        ...
        $this->layout = '';
        $files = scandir('./cork/carparks/', 1);
        if ($previousOrCurrent == 1) {
            $newest_file = $files[0];
            $json = file_get_contents('./cork/carparks/' . $newest_file);
            $json_array = json_decode($json, true);

            $newest_file2 = $files[1];
            $json2 = file_get_contents('./cork/carparks/' . $newest_file2);
            $json_array2 = json_decode($json2, true);


            $numOfCarparks = count(@$json_array['result']['records']);
            //$numOfCarparks = @$json_array['result']['records'].length;
            $cpFreeSpaces = 0;
            $cpFreeSpaces2 = 0;
            for ($i = 0; $i < $numOfCarparks; $i++) {

                $cpFreeSpaces = $cpFreeSpaces + @$json_array['result']['records'][$i]['free_spaces'];
